...
public class FinnhubProfileClient {
    ...
    public FinnhubProfileClient(HttpClient httpClient, String apiKey, String baseUrl) {
        ...
        if (k == null || k.isBlank()) throw new IllegalStateException("The environment variable 'FINNHUB_API_KEY' is not set.");
        this.apiKey = k;
    }

    /**
     * Minimal immutable holder for company profile fields returned by Finnhub /stock/profile2.
    ...
    public CompanyProfile getCompanyProfile(String symbol) throws IOException, InterruptedException {
        ...
        if (body == null || body.isBlank()) throw new RuntimeException(LOG_PREFIX + " empty profile response");
        ...
        Double marketCap = obj.has("marketCapitalization") && !obj.get("marketCapitalization").isJsonNull()
                ? obj.get("marketCapitalization").getAsDouble() : null;
        ...
        Double shareOut = obj.has("shareOutstanding") && !obj.get("shareOutstanding").isJsonNull()
                ? obj.get("shareOutstanding").getAsDouble() : null;
